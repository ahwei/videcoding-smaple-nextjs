...
export default function CartPage() {
  ...
  const handleQuantityChange = (drinkId: string, newQuantity: number) => {
    if (newQuantity <= 0) {
      removeItem(drinkId);
    } else {
      const updated = cartItems.map((item) =>
        item.drinkId === drinkId ? { ...item, quantity: newQuantity } : item
      );
  ...
  const removeItem = (drinkId: string) => {
    const updated = cartItems.filter((item) => item.drinkId !== drinkId);
    setCartItems(updated);
    localStorage.setItem("cart", JSON.stringify(updated));
  };

  const getCartItemDetails = (drinkId: string, quantity: number) => {
          ...
          <div className="pb-32">
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-6">
              {cartItems.map((item, index) => {
                const itemDetails = getCartItemDetails(item.drinkId, item.quantity);
                if (!itemDetails) return null;

                return (
                  <div key={item.drinkId} className={`p-6 ${index !== cartItems.length - 1 ? 'border-b border-gray-200' : ''}`}>
